...
// list pending users
router.get('/admin/pending-users', ver, async (req, res) => {
  const pending = await User.find({ isApproved: false });
  res.render('admin/pendingUsers', { pending });
});

// approve a user
router.post('/admin/approve/:id', ver, async (req, res) => {
...
// reject/delete
router.post('/admin/reject/:id', ver, async (req, res) => {
